              ...
              <form action="{{route('notes.update',$note)}}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <div class="radioG">
                        <h5> النوع :</h5>
                        <div class="radio">
                            <input type="radio" name="type" value="public" onclick="hideClasses()" {{$note->type ==='public' ? 'checked' : ''}} >
                            <label>عامة</label>
                        </div>
                        <div class="radio">
                            <input type="radio" name="type" value="private" onclick="showClasses()" {{$note->type ==='private' ? 'checked' : ''}}>
                            <label> خاصة</label>
                        </div>
                    </div>

                    <div id="class_select" @if($note->type=='public'){{'style=display:none'}} @endif>
                  <label for="class_id"><h5>الصف الدراسي:</h5></label>
                  <select name="class_id" id="class_id" class="form-control">
                  @foreach($classes as $class)
                  <option value="{{$class->id}}" {{ $note->class_id === $class->id ? "selected" : ""}}>{{$class->name}}</option>
                  @endforeach
                  </select>
